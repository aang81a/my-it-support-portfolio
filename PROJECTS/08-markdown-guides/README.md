# Markdown Support Guides

![Status](https://img.shields.io/badge/Status-In%20Progress-yellow)
![Focus](https://img.shields.io/badge/Focus-Markdown%20Documentation-blue)
![Skill](https://img.shields.io/badge/Skill-Technical%20Writing-blue)

---

## Overview

This section contains short Markdown-based quick-reference guides for common IT support tasks.

The guides are intentionally concise and focus on the core workflow of each task: purpose, steps, expected result, and important support notes.

---

## Purpose

The purpose of this section is to demonstrate clear technical documentation in Markdown format.

Unlike the full knowledge base articles, these guides are not written as polished end-user articles. They are short internal support notes that show how common procedures can be documented in a simple, structured, and consistent format.

---

## Topics Covered

- Domain password reset
- Domain join process
- Network troubleshooting
- Network drive mapping
- Active Directory user creation

---

## Skills Demonstrated

- Structured documentation
- Technical writing
- Step-by-step support instructions
- Internal support note writing
- IT support communication

---

## Guides

- [How to Reset a Domain Password](./how-to-reset-a-domain-password.md)
- [How to Join a PC to a Domain](./how-to-join-a-pc-to-a-domain.md)
- [How to Troubleshoot No Network Access](./how-to-troubleshoot-no-network-access.md)
- [How to Map a Network Drive](./how-to-map-a-network-drive.md)
- [How to Create a User in Active Directory](./how-to-create-a-user-in-active-directory.md)

